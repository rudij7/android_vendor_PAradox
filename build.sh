...
# Version 2.0.4, Adapted for XPerience-AOSP.
...
# Get build version
MAJOR=$(cat $DIR/vendor/PAradox/vendor.mk | grep 'ROM_VERSION_MAJOR := *' | sed  's/ROM_VERSION_MAJOR := //g')
MINOR=$(cat $DIR/vendor/PAradox/vendor.mk | grep 'ROM_VERSION_MINOR := *' | sed  's/ROM_VERSION_MINOR := //g')
MAINTENANCE=$(cat $DIR/vendor/PAradox/vendor.mk | grep 'ROM_VERSION_MAINTENANCE := *' | sed  's/ROM_VERSION_MAINTENANCE := //g')
TAG=$(cat $DIR/vendor/PAradox/vendor.mk | grep 'ROM_VERSION_TAG := *' | sed  's/ROM_VERSION_TAG := //g')
...
echo -e "${cya}Building ${bldcya}XPerience-AOSP $VERSION for $DEVICE ${txtrst}";
...
if [ -n "${INTERACTIVE}" ]; then
        ...
        if [ "${VENDOR}" == "PA" ]; then
                echo -e "[${bldgrn}lunch par_$DEVICE-userdebug${bldblu}]${txtrst}"
        else
